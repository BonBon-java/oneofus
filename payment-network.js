...
function paymentNetwork(env = process.env) {
  ...
  if (env.ONE_OF_US_PAYMENT_MODE === 'staging') {
    ...
    if (rpc.protocol !== 'https:' || tokenAddress === USDT_TOKEN_ADDRESS.toLowerCase() || tokenAddress === `0x${'0'.repeat(40)}`) throw new Error('Staging payments require HTTPS RPC and a non-zero non-production 6-decimal test token.');
