...
test('real PostgreSQL + local EVM payout lifecycle', { skip: !enabled && 'run npm run test:payout-integration:local' }, async (t) => {
  ...
  await t.test('concurrent workers can only submit the same persisted winner transaction', async () => {
    const settlement=await service().createSettlement(await seedWinnerSelected()); const winnerBefore=await token.balanceOf(winner); let sends=0; const counted=Object.create(provider); counted.recoverOrBroadcast=async (intent)=>{sends+=1;return provider.recoverOrBroadcast(intent);}; const concurrent=new SettlementService(repository,()=>counted);
    const [a,b]=await Promise.all([concurrent.executePayout(settlement.id),concurrent.executePayout(settlement.id)]); assert.ok(sends >= 1); assert.equal(a.legs[0].transactionHash,b.legs[0].transactionHash); await settle(settlement.id,concurrent); assert.equal((await token.balanceOf(winner))-winnerBefore,BigInt(settlement.winnerAmount)); console.log(`LOCAL_TX_CONCURRENT=${a.legs[0].transactionHash}`);
  });
  ...
  await t.test('ambiguous broadcast and a failed treasury leg recover without a second winner payout', async () => {
    const settlement=await service().createSettlement(await seedWinnerSelected()); const winnerBefore=await token.balanceOf(winner);
    let broadcastAttempts=0; const ambiguous=Object.create(provider); ambiguous.recoverOrBroadcast=async (intent)=>{ broadcastAttempts+=1; await provider.recoverOrBroadcast(intent); if (broadcastAttempts===1) throw new Error('response lost after node accepted transaction'); };
    const first=await new SettlementService(repository,()=>ambiguous).executePayout(settlement.id); assert.equal(first.legs[0].status,'signed'); assert.equal(first.legs[0].failureCode,'broadcast_ambiguous'); const winnerHash=first.legs[0].transactionHash;
    const recovered=await service().executePayout(settlement.id); assert.equal(recovered.legs[0].transactionHash,winnerHash); const winnerConfirmed=await service().confirmPayout(settlement.id); assert.equal(winnerConfirmed.status,'winner_confirmed'); assert.equal((await token.balanceOf(winner))-winnerBefore,BigInt(settlement.winnerAmount));
    const treasury=ethers.getAddress(process.env.TREASURY_WALLET_ADDRESS); const noGasForTreasury=Object.create(provider); noGasForTreasury.signTransfer=async ({recipient,amount})=>{ if (ethers.getAddress(recipient)===treasury) throw new PayoutPreflightError('insufficient_native_gas'); return provider.signTransfer({recipient,amount}); };
    const blocked=await new SettlementService(repository,()=>noGasForTreasury).executePayout(settlement.id); assert.equal(blocked.status,'blocked'); assert.equal(blocked.legs[0].transactionHash,winnerHash); assert.equal(blocked.legs[0].status,'confirmed'); assert.equal(blocked.legs[1].status,'failed');
    const treasurySubmitted=await service().executePayout(settlement.id); assert.equal(treasurySubmitted.status,'treasury_submitted'); assert.equal(treasurySubmitted.legs[0].transactionHash,winnerHash); const settled=await service().confirmPayout(settlement.id); assert.equal(settled.status,'settled'); assert.equal((await token.balanceOf(winner))-winnerBefore,BigInt(settlement.winnerAmount));
  });
