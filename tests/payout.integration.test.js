...
test('real PostgreSQL + local EVM payout lifecycle', { skip: !enabled && 'run npm run test:payout-integration:local' }, async (t) => {
  ...
  async function confirm(settlementId, instance = service()) { const result = await instance.confirmPayout(settlementId); assert.equal(result.status, 'confirmed'); }
  await t.test('happy path and PostgreSQL settlement uniqueness', async () => {
    ...
    const amount=BigInt(created.winnerAmount), senderBefore=await token.balanceOf(config.sender), winnerBefore=await token.balanceOf(winner); const broadcast=await service().executePayout(created.id); assert.equal(broadcast.status,'broadcast'); await confirm(created.id);
    assert.equal((await token.balanceOf(winner))-winnerBefore,amount); assert.equal(senderBefore-(await token.balanceOf(config.sender)),amount); assert.equal((await pool.query('SELECT status FROM rounds WHERE id=$1',[round])).rows[0].status,'completed'); console.log(`LOCAL_TX_HAPPY=${broadcast.transactionHash}`);
  });
  await t.test('concurrent workers broadcast exactly once', async () => {
    ...
    const [a,b]=await Promise.all([concurrent.executePayout(settlement.id),concurrent.executePayout(settlement.id)]); assert.equal(sends,1); assert.equal(a.transactionHash,b.transactionHash); await confirm(settlement.id,concurrent); console.log(`LOCAL_TX_CONCURRENT=${a.transactionHash}`);
  });
  await t.test('crash after real broadcast recovers signed transaction without a second payment', async () => {
    const settlement=await service().createSettlement(await seedWinnerSelected()); const before=await token.balanceOf(winner); const signed=await repository.preparePayout(settlement.id,provider); await provider.recoverOrBroadcast(signed);
    const afterRestart=service(); const recovered=await afterRestart.executePayout(settlement.id); assert.equal(recovered.transactionHash,signed.transactionHash); await confirm(settlement.id,afterRestart); assert.equal((await token.balanceOf(winner))-before,BigInt(settlement.winnerAmount)); console.log(`LOCAL_TX_RECOVERED=${signed.transactionHash}`);
  });
  await t.test('confirmation retry avoids rebroadcast and preflight failures fail closed', async () => {
    ...
    assert.equal((await repository.confirmPayout(settlement.id,{confirmed:false})).status,'broadcast'); assert.equal(sends,1); await confirm(settlement.id,retryService); assert.equal(sends,1);
    const fundedNoToken=new EthersTestnetPayoutProvider({privateKey:'0x5de4111afa1a4b94908f83103eb1f1706367c2e68ca870fc3fb9a804cdab365a'}); await assert.rejects(fundedNoToken.signTransfer({recipient:winner,amount:'1'}),(error)=>error instanceof PayoutPreflightError && error.code==='insufficient_test_token');
    const noGasWallet=ethers.Wallet.createRandom(); await (await new ethers.Contract(config.token,['function mint(address,uint256)'],provider.wallet).mint(noGasWallet.address,10n)).wait(); const noGas=new EthersTestnetPayoutProvider({privateKey:noGasWallet.privateKey}); await assert.rejects(noGas.signTransfer({recipient:winner,amount:'1'}),(error)=>error instanceof PayoutPreflightError && error.code==='insufficient_native_gas');
