...
test('orchestrator E2E: OPEN to COMPLETED with PostgreSQL, deterministic drand and real MockUSDT', { skip: !enabled && 'run npm run test:orchestrator-e2e:local' }, async () => {
  ...
  assert.equal(settlement.rowCount,1); assert.equal(String(settlement.rows[0].settlement_basis),total); assert.equal(String(settlement.rows[0].organizer_fee_accrued),'1500000'); assert.equal(String(settlement.rows[0].winner_amount),'8500000'); assert.equal(settlement.rows[0].status,'confirmed');
  assert.equal(intent.rowCount,1); assert.equal(intent.rows[0].status,'confirmed'); assert.ok(intent.rows[0].transaction_hash);
  const receipt = await payout.provider.getTransactionReceipt(intent.rows[0].transaction_hash); const transaction = await payout.provider.getTransaction(intent.rows[0].transaction_hash); assert.equal(receipt.status,1); assert.equal(ethers.getAddress(transaction.to),config.token);
  assert.equal((await token.balanceOf(winner))-winnerBefore,8500000n); assert.equal(signerBefore-(await token.balanceOf(config.sender)),8500000n);
  const verification = await repository.verifyDraw(id.round); assert.equal(verification.status,'verified'); assert.equal(verification.winningTicket,'1'); assert.equal(verification.publicRandomness,beacon.randomness); assert.equal(verification.payout.status,'confirmed'); assert.equal(verification.payout.transactionHash,intent.rows[0].transaction_hash);
  ...
  console.log(`ORCHESTRATOR_E2E round=${id.round} drand=${committedRound} tx=${intent.rows[0].transaction_hash}`); await pool.end();
});
