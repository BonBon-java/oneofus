...
class PostgresRepository {
  ...
  async reportingSettlements() { const result = await this.pool.query('SELECT * FROM reporting_settlement_summary ORDER BY created_at DESC LIMIT 200'); return result.rows.map((row) => ({ settlementId: row.settlement_id, roundId: row.round_id, status: row.status, grossPoolBaseUnits: String(row.gross_pool_base_units), winnerWallet: row.winner_wallet, winnerAmount: String(row.winner_amount), platformGrossFee: String(row.platform_gross_fee), treasuryWallet: row.treasury_wallet, treasuryAmount: row.treasury_amount === null ? null : String(row.treasury_amount), totalGasWei: String(row.total_gas_wei), createdAt: row.created_at, confirmedAt: row.confirmed_at })); }
  ...
  async createSettlement(roundId) {
    return this.transaction(async (db) => {
      ...
      // Legs give winner and treasury independent durable idempotency states.
      // Testnet configurations have no treasury; the leg remains unexecutable
      // until a production treasury is explicitly configured.
      if (treasuryWallet) await db.query(`INSERT INTO settlement_legs (id,settlement_id,kind,recipient_wallet,amount,chain_id,token_address) VALUES ($1,$2,'winner_payout',$3,$4,$5,$6),($7,$2,'treasury_sweep',$8,$9,$5,$6)`, [crypto.randomUUID(), settlementId, result.rows[0].winner_payout_wallet, amounts.winnerAmount, payoutTarget.chainId, payoutTarget.tokenAddress.toLowerCase(), crypto.randomUUID(), treasuryWallet, amounts.organizerFee]);
